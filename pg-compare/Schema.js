...
var Schema = function (db, options) {
  ...
  var getFunctions = function (callback) {
    ...
    db.query('SHOW server_version', []).then(
      function(result) {
        var serverVersion = result.rows[0].server_version
        var majorVersion = Number(serverVersion.split('.')[0])
        var aggFilter = majorVersion < 11 ? 'NOT p.proisagg' : 'NOT p.prokind = \'a\''
